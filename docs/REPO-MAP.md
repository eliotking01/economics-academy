...
| `images/` (311 live) | Photos, 106 note diagrams, and 84 hand-drawn SVG diagrams used by the flashcards. The SVGs are referenced only from `flashcards/data/*.json`, so a tool that greps HTML reports them as unused. They are not. Since 2026-08-23 every diagram PNG has a lossless `.webp` twin (`scripts/build_diagram_webp.py`, offered first through `<picture>`; the PNG stays the fallback at its URL) and the five photos have `.webp` and `-400.webp` variants the same way. |
| `webfonts/` (13 live) | The icon font, **trimmed to only the icons the site uses** (adding a new icon means adding its CSS rule *and* re-running the subsetter, or it renders as nothing at all, silently), and since 2026-08-23 the two text families self-hosted — nine static latin woff2 files as Google Fonts served them (Source Sans Pro ×6, Merriweather ×3), with their SIL OFL texts and `FONTS.txt` saying where each came from. Declared by `@font-face` in `css/main.css` (Source Sans Pro) and the two Merriweather sheets; no page links Google Fonts any more, and Open Sans is gone. |
