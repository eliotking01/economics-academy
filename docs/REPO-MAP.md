...
| `webfonts/` (12 live) | The icon font, **trimmed to only the icons the site uses** (adding a new icon means adding its CSS rule *and* re-running the subsetter, or it renders as nothing at all, silently), and since 2026-08-23 the three text families self-hosted — eleven static latin woff2 files as Google Fonts served them, with their SIL OFL texts and `FONTS.txt` saying where each came from. Declared by `@font-face` in `css/main.css` (Source Sans Pro, Open Sans) and the two Merriweather sheets; no page links Google Fonts any more. |
