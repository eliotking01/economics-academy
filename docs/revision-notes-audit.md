...
| **W2** | 22 occurrences across 16 pages                                                               | **RESOLVED.** `programs` in policy and commercial contexts — welfare, social, training and loyalty programmes — changed to `programme`. None was a computer program, the only sense in which `program` is correct British English. The site already used "structural adjustment programmes" correctly in `edexcel-theme-4/4-3-3`, so usage is now consistent at 23. |
| **W3** | `revision-notes/macroeconomics-diagrams.html`, `revision-notes/microeconomics-diagrams.html` | **RESOLVED.** Both used the entity `&rsaquo;` for their second breadcrumb separator while the first separator in the same nav, and all 501 elsewhere, were the literal `›`. All 503 separators are now identical in source. Same character, so nothing renders differently — verified by pixel diff.                                                                |
