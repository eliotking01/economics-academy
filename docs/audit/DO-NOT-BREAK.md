...
>
> **Amended 2026-08-23, topic-tail redesign (D58). The `notes-cta` box is
> gone from the 166 topic pages** — it is now only on the two diagram
> galleries and macro-application — and what this entry protects is intact:
> the board-named past-papers link is still on every topic page, per page,
> generated by `scripts/notes_extras.py` as the last sentence of the
> past-paper panel ("Whole papers and mark schemes: Edexcel past papers")
> or as the panel itself where the topic has no tagged questions; the
> `spec-alert` is untouched. The tail also now names the board in the derived
> past-paper line ("from the AQA A-Level papers") and in the flashcard deck
> name ("the AQA microeconomics deck"), so a topic page carries MORE board-
> specific chrome than before, not less. Do not make the tail board-generic.
>
