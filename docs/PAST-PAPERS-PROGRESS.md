...
| PDF tooling                   | Swift + PDFKit                                                                       | No Python PDF library, no `requirements.txt`, no venv in this repo; macOS ships PDFKit. Precedent set in `docs/QUESTIONS_PROGRESS.md` §7                                                                     |
| Tags separate from extraction | `tags.json` keyed by question id                                                     | Re-running the extractor must never destroy hand-tagging                                                                                                                                                |
| Progress file name            | `docs/PAST-PAPERS-PROGRESS.md`, not `PROGRESS.md`                                         | Matches the existing `_archive/PROJECT-LOG.md` / `docs/QUESTIONS_PROGRESS.md` convention                                                                                                                              |
...
1. **Internal links** from notes pages to their topic's questions page, following
   the additive-only pattern of `scripts/append_questions_link.py`. (Done, then
   superseded 2026-08-23: the notes tail is generated by `scripts/notes_extras.py`
   and derives its past-paper line from the bank's source files.)
