...
RETIRED 2026-08-23 - kept as a record, do not run. The topic-tail redesign
(docs/audit/DECISIONS.md D58) made the whole tail of every notes page
generated by scripts/notes_extras.py; the past-paper link's target is now
derived from the bank's source files and the volume gate on every build, so
there is nothing left to re-point. The text below describes the world before
that.
