...
So: every published page EXCEPT the four families named in GENERATED. Coverage
goes from 179 files to 192. If one of those four families' wording is wrong,
the two checks named above are what catch it.

CAUTION: THE 192 ARE NOT ALL HAND-WRITTEN, AND THIS PARAGRAPH USED TO SAY THEY
WERE. GENERATED lists four prefixes and `revision-notes/` is not one of them, so
all 173 notes pages are INSIDE the 192 - they became generated by
build_notes_pages.py in Phases 3 and 5, after this list was written, and the
list was never revisited. That is correct and deliberate to keep: the coverage
is what caught Wave 5.4's approved heading move on 1-1-2. Do not "fix" it by
adding `revision-notes/` to GENERATED. What was wrong was the word
hand-written, which sent one session looking for a different safety net; the
same staleness DO-NOT-BREAK records for page_shell.preconnectEarly.

A deliberate reordering inside a notes page therefore needs a Text-Change:
trailer, exactly like any other page.
